...
# Handle argument defaults BEFORE sourcing config.sh
# (config.sh uses $2 as config file, so we need to set it to default if not provided)
_ARG1="${1:-help}"
if [[ "$_ARG1" == "show" || "$_ARG1" == "redacted" ]] && [[ -z "$2" || "$2" != *.json ]]; then
    # For show command without explicit .json file, set default config
    # Preserve $2 (path) as $3
    set -- "$1" "$HOME/.picoclaw/config.json" "$2"
fi
...
case "$COMMAND" in
    ...
    commit)
        ...
        [ -f "$SECRETS_FILE" ] && cp "$SECRETS_FILE" "$BACKUP_DIR/.${BASE}_${TIMESTAMP}.secrets.json"
    ...
    show|redacted)
        # Output the staged config (optionally with jq path)
        # Usage: show [config.json] [jsonpath]
        #   or:   show [jsonpath] (uses default config)
        
        # Detect if $2 is a config file (ends .json) or a jsonpath
        if [[ "$2" == *.json ]]; then
            # Explicit config file given
            _config="$2"
            path="${3:-}"
        else
            # Default config, $2 is the path
            _config="$PICOCLAW_CONFIG"
            path="${2:-}"
        fi
        
        # Derive staging file from config path
        _base=$(basename "$_config" .json)
        _dir=$(dirname "$_config")
        _staging="$_dir/$_base.new.json"
    ...
    rollback)
        ...
        # Restore secrets if they exist for this backup
        LATEST_BASE=$(basename "$LATEST" .json)
        LATEST_SECRETS="$BACKUP_DIR/.$LATEST_BASE.secrets.json"
        
        cp "$LATEST" "$PICOCLAW_CONFIG"
        [ -f "$LATEST_SECRETS" ] && cp "$LATEST_SECRETS" "$SECRETS_FILE"
